  ...
  class ExpoPushNotification < ::Jobs::Base
    ...
    def execute(args)
      if args[:retry_ids].blank?
        ...
        expo_pn_subscriptions =
          ExpoPnSubscription.where(user_id: args[:user_id])
        push_notification =
          ::PushNotification.create(
            ...
            is_direct_message_channel: payload[:is_direct_message_channel],
        ...
        push_notification_retries =
          PushNotificationRetry.where(id: args[:retry_ids])
        # group by push_notification_id
        retries_by_push_notification_id =
          push_notification_retries.group_by(&:push_notification_id)
        ...
        retries_by_push_notification_id.each do |_push_notification_id, retries|
          ...
          expo_pn_subscriptions =
            ExpoPnSubscription.where(expo_pn_token: retries.map(&:token))
